...
public class MatchServiceImpl implements MatchService{
    ...
    @Value("${my.CrickBuzz.url}")
    private String thirdPartyURL;

    @Value("${points.table.url}")
    private String pointsTableURL;

    //returns match history of all matches in our DB
    @Override
    ...
    private void updateMatchDB(Match match1) {
        ...
        if(match==null){    // not present in DB hence saving it.
            matchRepo.save(match1);
        } else{      // already present in DB therefore setting the match ID as older one and updating other fields as new.
